...
class UploadFailed(RuntimeError):
    """An Azure upload completed unsuccessfully and can safely be retried."""
...
class AzureFileUploader:
            ...
            try:
                ...
                if self.verify_upload:
                    ...
                    if remote_size != local_size:
                        raise UploadFailed(
                            f"Remote size mismatch for {self.container}/{self.filename}: "
                ...
                if attempt == self.max_attempts:
                    logger.error(
                        "Upload failed after %s attempt(s) for %s/%s: %s",
                        attempt,
                        self.container,
                        self.filename,
                        exc,
                ...
                logger.warning(
                    "Upload attempt %s/%s failed for %s/%s; retrying in %.1fs: %s",
                    attempt,
                    self.max_attempts,
                    self.container,
                    self.filename,
    ...
    def wait_for_upload(self, *, raise_on_error=False):
        ...
        if raise_on_error and self._upload_error:
            raise UploadFailed(
                f"Upload failed for {self.container}/{self.filename}"
